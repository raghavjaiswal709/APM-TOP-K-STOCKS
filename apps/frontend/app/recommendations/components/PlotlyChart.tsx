...
const [chartType, setChartType] = useState<'line' | 'candle'>('line'); // ✅ NEW - Default to line
...
// const prepareLineChartData = () => {
//   // ✅ FIXED: Filter data to trading hours only
//   const filteredHistoricalData = historicalData.filter(point => 
//     isWithinTradingHours(point.timestamp)
//   );
  
//   const allData = [...filteredHistoricalData];
  
//   if (data && data.ltp && isWithinTradingHours(data.timestamp)) {
//     const lastPoint = allData.length > 0 ? 
//       allData[allData.length - 1] : null;
...
const calculateStandardDeviation = (values: number[]): number => {
  try {
    ...
    // Calculate variance (using sample standard deviation formula)
    const variance = validValues.reduce((sum, value) => {
      const diff = value - mean;
      return sum + (diff * diff);
    }, 0) / (validValues.length - 1); // Sample standard deviation
...
const calculateVWAP = (close: number[], high: number[], low: number[], volume: number[]) => {
 ...
 const prepareCandlestickData = () => {
  ...
  console.log('🔍 Raw OHLC data points:', ohlcData.length);
  
  // ✅ ENHANCED: Filter with better validation
  ...
  console.log('✅ Valid OHLC data points:', validOhlcData.length);
  ...
  const result = {
    ...
    volume: sortedData.map(candle => Number(candle.volume || 0)),
    volumeStdDev: sortedData.map((candle, index) => calculateVolumeStandardDeviation(candle, index)),
    buyVolumes: sortedData.map(candle => calculateBuySellVolume(candle).buyVolume),
    sellVolumes: sortedData.map(candle => calculateBuySellVolume(candle).sellVolume)
  };
  
  console.log('📊 Prepared candlestick data sample:', {
...
//   };
  ...
  // Get the current or data date
  const currentTime = data?.timestamp 
    ? new Date(data.timestamp * 1000) 
    : new Date();
    
  // Create trading day boundaries (9:15 AM to 3:30 PM IST)
  ...
  // For recorded data, use the date from selectedDate if available
  if (data?.timestamp) {
  ...
  // Handle different timeframes within trading hours
  ...
  // Ensure we don't go beyond trading hours
...
const createPlotData = () => {
  ...
  // ✅ PROFESSIONAL: Clean candlestick implementation
  ...
  for (let i = 0; i < x.length; i++) {
    // Only check for positive values, not OHLC relationships
  ...
  // ✅ INDICATORS for candlestick (using validData arrays)
...
// };
  ...
  // ✅ FIXED: Simplified domain calculation
  let mainChartDomain = [0, 1];
  if (showIndicators.rsi && showIndicators.macd) {
  ...
  const layout: any = {
    ...
    xaxis: {
      ...
      fixedrange: false,
      tickformat: '%H:%M',
      dtick: 30 * 60 * 1000, // 30-minute intervals
    },
    yaxis: {
      title: 'Price (₹)',
      range: yRange,
  ...
  // ✅ FIXED: Add RSI y-axis if needed
  if (showIndicators.rsi) {
    layout.yaxis2 = {
      title: 'RSI',
  ...
  // ✅ FIXED: Add MACD y-axis if needed
  if (showIndicators.macd) {
    layout.yaxis4 = {
      title: 'MACD',
      titlefont: { color: colors.text, size: 10 },
